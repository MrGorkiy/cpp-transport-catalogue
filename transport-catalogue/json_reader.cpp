...
bool JsonReader::FillTransportCatalogue() {
    ...
    for (const auto &stop: raw_stops_) {
        for (const auto &[other, distance]: stop.distances) {
            ...
            if (!pairOK) {
                std::cerr << "ERROR while adding distance to stop pair of "s << stop.stop_name << " and "s << other
                          << "." << std::endl;
            }
        }
    }
    ...
    for (auto &route: raw_buses_) {
        ...
        if (route.route_stops.size() < 2) {
            std::cerr << "Error while adding bus routes for bus: "s << route.bus_name
...
json::Node JsonReader::ProcessOneUserRequestNode(const json::Node &user_request) {
    ...
    if (const auto id_i = request_fields.find("id"s); id_i != request_fields.end() && id_i->second.IsInt()) {
        ...
        throw json::ParsingError(
                "Error reading JSON data with user requests to database. One of node's fields is crippled.");
    }
    ...
    if (type == "Route"s) {
        ...
        if (const auto from_it = request_fields.find("from"s); from_it != request_fields.end() &&
                                                               from_it->second.IsString()) {
            from_stop = from_it->second.AsString();
        } else {
            throw json::ParsingError(
                    "Error reading JSON data with user requests to database. Route->from field is crippled.");
        }
        if (const auto to_it = request_fields.find("to"s); to_it != request_fields.end() && to_it->second.IsString()) {
            to_stop = to_it->second.AsString();
        } else {
            throw json::ParsingError(
                    "Error reading JSON data with user requests to database. Route->to field is crippled.");
        }
    ...
    throw json::ParsingError(
            "Error reading JSON data with user requests to database. Node's type field contains invalid data.");
}
...
json::Node JsonReader::GenerateBusNode(int id, std::string &name) const {
    ...
    return json::Builder().StartDict().Key("request_id"s).Value(id).Key("curvature"s).Value(bi.curvature)
            .Key("route_length"s).Value(static_cast<int>(bi.route_length)).Key("stop_count"s).Value(
                    static_cast<int>(bi.stops_number))
...
RendererSettings JsonReader::GetRendererSetting() const {
    ...
    if (const auto width_i = render_settings.find("width"s); width_i != render_settings.end() &&
    ...
    if (const auto height_i = render_settings.find("height"s); height_i != render_settings.end() &&
                                                               height_i->second.IsDouble()) {
        settings.height = height_i->second.AsDouble();
    } else {
        throw json::ParsingError("Error while parsing renderer settings, height data.");
    }

    if (const auto padd_i = render_settings.find("padding"s); padd_i != render_settings.end() &&
                                                              padd_i->second.IsDouble()) {
        settings.padding = padd_i->second.AsDouble();
    } else {
        throw json::ParsingError("Error while parsing renderer settings, padding data.");
    }

    if (const auto field_iter = render_settings.find("line_width"s); field_iter != render_settings.end() &&
                                                                     field_iter->second.IsDouble()) {
    ...
    if (const auto field_iter = render_settings.find("stop_radius"s); field_iter != render_settings.end() &&
                                                                      field_iter->second.IsDouble()) {
    ...
    if (const auto field_iter = render_settings.find("bus_label_font_size"s); field_iter != render_settings.end() &&
                                                                              field_iter->second.IsInt()) {
    ...
    if (const auto field_iter = render_settings.find("stop_label_font_size"s); field_iter != render_settings.end() &&
                                                                               field_iter->second.IsInt()) {
    ...
    if (const auto field_iter = render_settings.find("underlayer_width"s); field_iter != render_settings.end() &&
                                                                           field_iter->second.IsDouble()) {
...
RoutingSettings JsonReader::GetRoutingSettings() const {
    ...
    if (const auto &bus_velocity = routing_settings.find("bus_velocity"); bus_velocity != routing_settings.end() &&
                                                                          bus_velocity->second.IsDouble()) {
...
JsonReader::GenerateRoute(std::string_view from_stop, std::string_view to_stop) const {
