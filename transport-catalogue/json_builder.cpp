...
namespace json {
        ...
        if (DoingDict()) {
            if (!curr_key_) {
                throw std::logic_error(
                        "Error in dictionary: key is not declared, can not start another dictionary as a value."s);
            }
    ...
    Builder &Builder::EndDict() {
        ...
        if (Node *node = nodes_stack_.back(); node->IsString()) {
        ...
        if (Node *node = nodes_stack_.back(); node->IsArray()) {
